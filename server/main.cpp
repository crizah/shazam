...
unordered_map<uint32_t, vector<vector<uint32_t>>> fingerPrint(vector<Peak> &peaks, uint32_t &songID, int range=5){
    ...
    for(int i=0; i<peaks.size(); i++){
        ...
        for(int j =i+1; j <= i + range && j<peaks.size(); j++){
            ...
            vector<uint32_t> val = {anchor_time, songID};

            // if same songID and hash exists, donot add to fp

            const auto& existing = fp[hash_i];
            bool exists = any_of(existing.begin(), existing.end(),
                                [&](const vector<uint32_t>& entry) {
                                return entry.size() == 2 && entry[0] == anchor_time && entry[1] == songID;
                                });
